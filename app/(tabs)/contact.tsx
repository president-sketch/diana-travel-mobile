...
export default function ContactScreen() {
  ...
  const handleSend = async () => {
    ...
    setIsPending(true);
    try {
      const res = await fetch("https://fdianatravel.com/api/contact", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: trimmedName, email: trimmedEmail, subject: "App Inquiry", message: trimmedMessage }),
      });
      if (res.ok) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ContactItem icon="phone" label="Line 1" value="01900596" onPress={() => call("01900596")} />
        <ContactItem icon="phone" label="Line 2" value="01900597" onPress={() => call("01900597")} />
        ...
        <ContactItem icon="mail" label="Email" value="info@fdianatravel.com" onPress={openEmail} />
        ...
        <Text style={[styles.sectionTitle, { color: colors.foreground, marginTop: 16 }]}>Send Message</Text>
        <View style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.formField}>
            ...
            <TextInput
              ...
              value={form.name}
              onChangeText={(text) => { setForm((prev) => ({ ...prev, name: text })); if (submitStatus !== "idle") setSubmitStatus("idle"); }}
          ...
          <View style={styles.formField}>
            ...
            <TextInput
              ...
              placeholderTextColor={colors.mutedForeground}
              value={form.email}
              onChangeText={(text) => { setForm((prev) => ({ ...prev, email: text })); if (submitStatus !== "idle") setSubmitStatus("idle"); }}
          ...
          <View style={styles.formField}>
            ...
            <TextInput
              ...
              placeholderTextColor={colors.mutedForeground}
              value={form.message}
              onChangeText={(text) => { setForm((prev) => ({ ...prev, message: text })); if (submitStatus !== "idle") setSubmitStatus("idle"); }}
              multiline
              numberOfLines={4}
              returnKeyType="done"
              blurOnSubmit
          ...
          {submitStatus === "success" && (
            <View style={styles.feedbackRow}>
              ...
              <Text style={[styles.feedbackText, { color: "#16a34a" }]}>Message sent! We'll get back to you within 24 hours.</Text>
            </View>
          )}
          {submitStatus === "error" && (
            <View style={styles.feedbackRow}>
          ...
          <Pressable
            style={({ pressed }) => [styles.sendBtn, { backgroundColor: "#D13434", opacity: pressed || isPending ? 0.8 : 1 }]}
            onPress={handleSend}
            disabled={isPending}
          >
            {isPending ? (
        ...
        <Pressable
          style={({ pressed }) => [styles.ctaBtn, { backgroundColor: "#25D366", opacity: pressed ? 0.85 : 1 }]}
          onPress={openWhatsApp}
        >
...
const styles = StyleSheet.create({
  ...
  heroAr: { fontSize: 16, color: "rgba(255,255,255,0.8)", marginBottom: 8, fontFamily: "Cairo_700Bold" },
  heroDesc: { fontSize: 14, color: "rgba(255,255,255,0.7)", lineHeight: 20 },
  sectionTitle: { fontSize: 13, fontWeight: "700", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8, marginTop: 4, opacity: 0.5 },
  contactItem: { flexDirection: "row", alignItems: "center", gap: 14, borderRadius: 14, padding: 16, marginBottom: 8, borderWidth: 1 },
  contactIcon: { width: 46, height: 46, borderRadius: 12, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  contactLabel: { fontSize: 12, fontWeight: "500", marginBottom: 2 },
  contactValue: { fontSize: 15, fontWeight: "600", lineHeight: 20 },
  formCard: { borderRadius: 16, padding: 16, borderWidth: 1, marginBottom: 8, gap: 12 },
  formField: { gap: 6 },
  ...
  sendBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 12, paddingVertical: 14, marginTop: 4 },
  sendBtnText: { color: "#fff", fontSize: 15, fontWeight: "700" },
  ctaBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 16, paddingVertical: 16, marginTop: 20 },
  ctaBtnText: { color: "#fff", fontSize: 16, fontWeight: "700", fontFamily: "Cairo_700Bold" },
});
